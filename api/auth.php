...
case 'payOrderCredit':
    ...
    try {
        ...
        $reason = "Credit payment for order #{$order['id']} ({$order['product_name']})";
        $stmt->execute([
            ':user_id' => $loggedInUserId,
            ':org_id' => $organizationId,
            ':amount' => $amount,
        ...
        $pdo->commit();
        recordActivity($pdo, $loggedInUserId, 'credit_payment', 
            "Paid $amount for order #{$order['id']}", $organizationId);
